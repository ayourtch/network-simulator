...
async fn test_end_to_end_packet_processing() {
    ...
    cfg.topology.routers.insert("Rx0y0".to_string(), toml::Value::Table(toml::map::Map::new()));
    cfg.topology.routers.insert("Rx0y1".to_string(), toml::Value::Table(toml::map::Map::new()));
    cfg.topology.links.insert(
        "Rx0y0_Rx0y1".to_string(),
        network_simulator::topology::link::LinkConfig { mtu: None, delay_ms: 0, jitter_ms: 0, loss_percent: 0.0, load_balance: false },
    );
    // Build fabric
    ...
    let processed = process_packet(&mut fabric, &tables, RouterId("Rx0y0".to_string()), packet.clone(), Destination::TunB).await;
