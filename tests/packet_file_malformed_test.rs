...
#[test]
fn test_cli_packet_file_malformed() {
    // Create a temporary packet file with an invalid line followed by a valid packet
    let mut tmp_packet = NamedTempFile::new().expect("temp packet file");
    writeln!(tmp_packet, "invalidhex").expect("write invalid line");
...
"#;
    let cfg_path = "tests/tmp_config_malformed.toml";
