...
fn test_multipath_forwarding_load_balance() {
    ...
    let cfg_lb = LinkConfig { mtu: None, delay_ms: 1, jitter_ms: 0, loss_percent: 0.0, load_balance: true };
    fabric.add_link(&RouterId("Rx0y0".to_string()), &RouterId("Rx0y1".to_string()), cfg_lb.clone());
    fabric.add_link(&RouterId("Rx0y0".to_string()), &RouterId("Rx0y2".to_string()), cfg_lb.clone());
